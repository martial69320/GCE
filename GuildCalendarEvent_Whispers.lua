...
----------------------------------------
GuildCalendarEvent._WhisperLog = {}
----------------------------------------

function GuildCalendarEvent._WhisperLog:Construct()
	self.Players = {}

	GuildCalendarEvent.EventLib:RegisterEvent("CHAT_MSG_WHISPER", function (pMessage, pPlayerName) self:AddWhisper(pPlayerName, pMessage) end)
end

function GuildCalendarEvent._WhisperLog:AddWhisper(pPlayerName, pMessage)
	-- If no event is active then just ignore all whispers
	
	if not GuildCalendarEvent.RunningEvent then
		return
	end
	...
	if not vPlayerLog then
		...
		vPlayerLog.Date, vPlayerLog.Time = GuildCalendarEvent.DateLib:GetServerDateTime60()
		vPlayerLog.Whispers = {}
...
function GuildCalendarEvent._WhisperLog:AskClear()
	StaticPopup_Show("CONFIRM_CALENDAR_CLEAR_WHISPERS")
end

function GuildCalendarEvent._WhisperLog:Clear()
	self.Players = {}
...
function GuildCalendarEvent._WhisperLog:GetPlayerWhispers(pEvent)
	if GuildCalendarEvent.RunningEvent ~= pEvent then
		return
	end
...
function GuildCalendarEvent._WhisperLog:SetNotificationFunc(pFunc, pParam)
	self.NotificationFunc = pFunc
	self.NotifcationParam = pParam
end

function GuildCalendarEvent._WhisperLog:SetWhisperFilterFunc(pFunc, pParam)
	self.WhisperFilterFunc = pFunc
	self.WhisperFilterParam = pParam
end

function GuildCalendarEvent._WhisperLog:RemovePlayer(pPlayerName)
...
function GuildCalendarEvent._WhisperLog:GetNextWhisper(pPlayerName)
