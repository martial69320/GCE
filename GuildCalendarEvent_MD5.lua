...
-- Transformed for use in Guild Event Calendar by John Stephen

GuildCalendarEvent._MD5 = {}

GuildCalendarEvent._MD5.Mask32 = tonumber("ffffffff", 16)
GuildCalendarEvent._MD5.Consts =
{
...
for vIndex, vConstStr in ipairs(GuildCalendarEvent._MD5.Consts) do
	GuildCalendarEvent._MD5.Consts[vIndex] = tonumber(vConstStr, 16)
end

function GuildCalendarEvent._MD5:Construct()
end

function GuildCalendarEvent._MD5.f(x, y, z)
	return bit.bor(bit.band(x, y), bit.band(-x - 1, z))
end

function GuildCalendarEvent._MD5.g(x, y, z)
	return bit.bor(bit.band(x, z), bit.band(y, -z - 1))
end

function GuildCalendarEvent._MD5.h(x, y, z)
	return bit.bxor(x, bit.bxor(y, z))
end

function GuildCalendarEvent._MD5.i(x, y, z)
	return bit.bxor(y, bit.bor(x, -z - 1))
end

function GuildCalendarEvent._MD5:z(f,a,b,c,d,x,s,ac)
...
function GuildCalendarEvent._MD5:Transform(A,B,C,D,X)
...
function GuildCalendarEvent._MD5:Calc(pString)
	self:BeginDigest()
	self:DigestString(pString)
	return self:EndDigest()
end

function GuildCalendarEvent._MD5:BeginDigest()
	self.CurMessage = ""
	self.CurLen = 0
	self.TotalLen = 0
...
function GuildCalendarEvent._MD5:DigestString(pString)
...
function GuildCalendarEvent._MD5:EndDigest()
...
function GuildCalendarEvent._MD5.swap(w)
	local vResult = GuildCalendarEvent._MD5.StringToBE4(GuildCalendarEvent._MD5.LEToString(w))
...
function GuildCalendarEvent._MD5.LEToString(i)
	i = math.floor(i)
...
function GuildCalendarEvent._MD5.StringToBE4(str)
	return 256 * (256 * (256 * strbyte(str, 1) + strbyte(str, 2)) + strbyte(str, 3)) + strbyte(str, 4)
end

-- cut up a string in little-endian ints of given size

function GuildCalendarEvent._MD5.SliceStringToLEs(s)
	local r = {}
	local o = 1
...
function GuildCalendarEvent._MD5:Verify()
	s0 = 'message digest'
	...
	if self:Calc('') ~= 'd41d8cd98f00b204e9800998ecf8427e' then
		GuildCalendarEvent:ErrorMessage("MD5: Test 1 failed")
		return
	end
	
	if self:Calc('a') ~= '0cc175b9c0f1b6a831c399e269772661' then
		GuildCalendarEvent:ErrorMessage("MD5 test 2 failed")
		return
	end
	
	if self:Calc('abc') ~= '900150983cd24fb0d6963f7d28e17f72' then
		GuildCalendarEvent:ErrorMessage("MD5 test 3 failed")
		return
	end
	
	if self:Calc(s0) ~= 'f96b697d7cb7938d525a2f31aaf161d0' then
		GuildCalendarEvent:ErrorMessage("MD5 test 4 failed")
		return
	end
	
	if self:Calc(s1) ~= 'c3fcd3d76192e4007dfb496cca67e13b' then
		GuildCalendarEvent:ErrorMessage("MD5 test 5 failed")
		return
	end
	
	if self:Calc(s2) ~= 'd174ab98d277d9f5a5611c2c9f419d9f' then
		GuildCalendarEvent:ErrorMessage("MD5 test 6 failed")
		return
	end
	
	if self:Calc(s3) ~= '57edf4a22be3c955ac49da2e2107b67a' then
		GuildCalendarEvent:ErrorMessage("MD5 test 7 failed")
		return
	end
	
	GuildCalendarEvent:NoteMessage("MD5: All tests passed")
end

function GuildCalendarEvent._MD5:CheckPerformance()
	...
	for _, vSize in ipairs(sizes) do
		...
		GuildCalendarEvent:NoteMessage("%6d bytes: %4d ms", vSize, vElapsed)
	end
end
